...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        ...
        count = 0
        #max_length = max(len(source_words), len(typed_words))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if diff_function(typed_word, min_diff,limit) > limit or typed_word in word_list:
        return typed_word
    else:
        return min_diff
    # END PROBLEM 5
...
def fastest_words(match):
    ...
    ret = [[] for i in range(len(player_indices))]
    for i in word_indices:
        fast_player = min(player_indices, key=lambda x: times[x][i])
        ret[fast_player].append(get_word(match, i))
    return ret
    # END PROBLEM 10
